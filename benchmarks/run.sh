...
# prints one Markdown table per sub-benchmark: ns/op and the ratio vs MRI. Before
# any timing, the drivers' VERIFY digests are checked byte-identical to MRI.
#
...
# --- Correctness gate: Go VERIFY digests must equal MRI's, byte-for-byte -------
echo "  verifying Go output == MRI ..." >&2
( cd go && GOWORK=off go run . 2>/dev/null ) | grep '^VERIFY' | sort > "$VER.go"
"$RUBY" "$RB" 2>/dev/null | grep '^VERIFY' | sort > "$VER.mri"
if ! diff -q "$VER.go" "$VER.mri" >/dev/null; then
  echo "  MISMATCH — Go output differs from MRI; aborting:" >&2
  diff "$VER.mri" "$VER.go" >&2
  exit 1
fi
echo "  OK: Go output byte-identical to MRI" >&2
